...
class RDTInferencer:
    ...
    def encode_image_and_instruction(self, images, instruction):
        ...
        with torch.no_grad():
            ...
            if isinstance(self.config["model"]["selected_layers"], list):
                vlang_hidden = torch.stack(
                    [outputs.hidden_states[i]
                     for i in self.config["model"]["selected_layers"]],
                    dim=1,
                )  # (1, depth, seq_len, vlm_hidden_size)
            else:
                vlang_hidden = outputs.hidden_states[
                    self.config["model"]["selected_layers"]]
                # (1, seq_len, vlm_hidden_size)

        return vlang_hidden, vlang_attn_mask
    ...
    def step(self, observations, instruction):
        ...
        vlang_hidden, vlang_attn_mask = self.encode_image_and_instruction(images, instruction)

        # Predict the next action chunk given the inputs
        nsample = self.policy.predict_action(
            lang_tokens=vlang_hidden,
