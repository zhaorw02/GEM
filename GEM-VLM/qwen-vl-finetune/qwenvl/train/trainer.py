...
def patched_log(self, logs, start_time=None):
    ...
    if hasattr(self, "_train_t0") is False:
        self._train_t0 = now
    ...
    eta_hour = None
    if step > 0 and self.state.max_steps and self.state.max_steps > 0:
        avg = (now - self._train_t0) / step
        eta_hour = int(max(self.state.max_steps - step, 0) * avg) / 3600

    logs = dict(logs)  # make sure it's mutable
    logs["step"] = step
    logs["step_time_sec"] = round(step_time, 4)
    if eta_hour is not None:
        logs["eta_hour"] = round(eta_hour, 2)

    return _ORIG_LOG(self, logs, start_time=start_time)
...
def gather_log_metrics_sum(metrics):
    if torch.distributed.is_initialized():
        ...
        metrics = {k: [o[k] for o in output if o is not None] for k in metrics.keys()}

        for k in metrics.keys():
            # if 'loss' in k or 'code_correct_percent' in k:
            if 'loss' in k or 'codes_kept_ratios' in k or 'code_correct_percent' in k:
                # metrics[k] = metrics[k] / torch.distributed.get_world_size()
                total = sum([1 if item != 0.0 else 0 for item in metrics[k]])
                if total != 0:
                    metrics[k] = sum(metrics[k]) / sum([1 if item != 0.0 else 0 for item in metrics[k]])
                else:
                    metrics[k] = sum(metrics[k]) / 1
...
def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
    ...
    dit_loss = outputs.dit_loss
    if dit_loss is None:
        dit_loss = 0.0
    else:
        logs['dit_loss'] = dit_loss.item()
